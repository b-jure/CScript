...
#define val2tbmask(v)                                                                              \
    cast_uint(                                                                                     \
        0 | (IS_NIL(v) * 1) | (IS_NUMBER(v) * 2) | (IS_STRING(v) * 4) | (IS_BOOL(v) * 8) |         \
        (IS_CLASS(v) * 16) | (IS_INSTANCE(v) * 32) |                                               \
        ((IS_FUNCTION(v) || IS_CLOSURE(v) || IS_NATIVE(v) || IS_BOUND_METHOD(v)) * 64))

#define val2tbmask_1(v)                                                                            \
    cast_uint(0 | (IS_NIL(v) * 1) | (IS_NUMBER(v) * 2) | (IS_BOOL(v) * 4) | (IS_OBJ(v) * 8))
#endif
