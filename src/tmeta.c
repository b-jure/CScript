...
#include "tscriptprefix.h"

#include "tmeta.h"
#include "tlist.h"
#include "tlexer.h"
#include "tscriptconf.h"
#include "tstring.h"
#include "tdebug.h"
#include "tstate.h"
#include "ttable.h"
...
TOKUI_DEF const char *const csO_typenames[TOKUI_TOTALTYPES] = {
...
void csMM_init(toku_State *T) {
    const char *mmnames[TOKU_MT_NUM] = { /* ORDER MT */
...
OClass *csMM_newclass(toku_State *T) {
    GCObject *o = csG_new(C, sizeof(OClass), TOKU_VCLASS);
...
Instance *csMM_newinstance(toku_State *T, OClass *cls) {
    GCObject *o = csG_new(C, sizeof(Instance), TOKU_VINSTANCE);
...
UserData *csMM_newuserdata(toku_State *T, size_t size, int nuv) {
    GCObject *o;
    UserData *ud;
    if (t_unlikely(size > TOKU_MAXSIZE - udmemoffset(nuv)))
        csM_toobig(C);
    o = csG_new(C, sizeofuserdata(nuv, size), TOKU_VUSERDATA);
    ud = gco2u(o);
    ud->metalist = NULL;
    ud->methods = NULL;
    ud->nuv = nuv;
...
IMethod *csMM_newinsmethod(toku_State *T, Instance *ins, const TValue *method) {
    GCObject *o = csG_new(C, sizeof(IMethod), TOKU_VIMETHOD);
    IMethod *im = gco2im(o);
    im->ins = ins;
...
UMethod *csMM_newudmethod(toku_State *T, UserData *ud, const TValue *method) {
    GCObject *o = csG_new(C, sizeof(UMethod), TOKU_VUMETHOD);
    UMethod *um = gco2um(o);
    um->ud = ud;
...
/* get method 'mm' */
const TValue *csMM_get(toku_State *T, const TValue *v, int mm) {
    List *ml;
...
*/
const char *csMM_objtypename(toku_State *T, const TValue *o) {
    List *ml;
    if ((ttisinstance(o) && (ml = insval(o)->oclass->metalist)) ||
        (ttisfulluserdata(o) && (ml = udval(o)->metalist))) {
        const TValue *v = csA_getival(C, ml, TOKU_MT_NAME);
        if (ttisstring(v))
...
void csMM_callset(toku_State *T, const TValue *f, const TValue *o,
...
void csMM_callgetres(toku_State *T, const TValue *f, const TValue *o,
...
void csMM_callbinres(toku_State *T, const TValue *f, const TValue *o1,
...
void csMM_trybin(toku_State *T, const TValue *v1, const TValue *v2,
                              SPtr res, int mm) {
    if (t_unlikely(!callbinMM(C, v1, v2, res, mm)))
        csD_binoperror(C, v1, v2, mm);
}


void csMM_callunaryres(toku_State *T, const TValue *fn,
...
void csMM_tryunary(toku_State *T, const TValue *o, SPtr res, int mm) {
    if (t_unlikely(!callunMM(C, o, res, mm))) {
        TValue dummy;
        setival(&dummy, 0);
...
void csMM_tryconcat(toku_State *T) {
    SPtr p1 = C->sp.p - 2; /* first argument */
    if (t_unlikely(!callbinMM(C, s2v(p1), s2v(p1 + 1), p1, TOKU_MT_CONCAT)))
        csD_concaterror(C, s2v(p1), s2v(p1 + 1));
}


/* call order method */
int csMM_order(toku_State *T, const TValue *v1, const TValue *v2, int mm) {
    toku_assert(toku_MT_EQ <= mm && mm <= toku_MT_NUM);
    if (t_likely(callbinMM(C, v1, v2, C->sp.p, mm)))
...
** immediate value.
*/
int csMM_orderI(toku_State *T, const TValue *v1, int v2, int flip, int isflt,
                int mm) {
    const TValue *v2_;
    TValue aux;
    if (isflt) {
