...
{
	if (likely(cast_int(vm->sp - vm->stack) < VM_STACK_LIMIT))
		*vm->sp++ = val;
	else
		sovferror(vm);
}
...
{
	Value method;
	if (!tableget(vm, &oclass->methods, name, &method))
		return 0;
	*stkpeek(0) = OBJ_VAL(OBoundMethod_new(vm, receiver, AS_CLOSURE(method)));
	return 1;
}
...
l_callstack_overflow:
	fcovferror(vm);
#else
	if (unlikely(!cr_ensurestack(vm, retcnt))) {
		retovferror(vm, p->name->storage);
...
static force_inline void invokefrom(VM *vm, OClass *oclass, Value name, int32_t argc, int32_t retcnt)
{
	Value method;
	if (unlikely(!rawget(vm, &oclass->methods, name, &method)))
...
l_call:;
			...
			CASE(OP_METHOD)
			{
				...
				OClass *oclass = AS_CLASS(*stkpeek(1));
...
l_invoke:;
			...
			CASE(OP_GET_SUPER)
			{
				cr_assert(vm, IS_CLASS(*stkpeek(0)), "Expect OClass.");
				Value methodname = READ_CONSTANT();
				OClass *superclass = AS_CLASS(pop(vm));
				saveip();
				if (unlikely(!bindmethod(vm, superclass, methodname, *stkpeek(0))))
...
l_invoke_super:;
					{
						Value methodname = READ_CONSTANT();
						int32_t retcnt = READ_BYTEL();
						cr_assert(vm, IS_CLASS(*stkpeek(0)), "superclass must be class.");
						OClass *superclass = AS_CLASS(pop(vm));
						saveip();
...
l_set_global:;
				{
					...
					if (unlikely(gvar->value == EMPTY_VAL)) {
						saveip();
						udgerror(vm, globalname(vm, bytecode_param)->storage);
					} else if (unlikely(VISCONST(gvar))) {
						saveip();
...
l_set_local:;
			...
			{
				cr_assert(vm, IS_CLASS(*stkpeek(0)), "subclass must be class.");
				OClass *subclass = AS_CLASS(*stkpeek(0));
				...
				memcpy(subclass->omethods, AS_CLASS(superclass)->omethods, sizeof(subclass->omethods));
				pop(vm); // pop subclass
				BREAK;
			}
