...
criptapi cr_ubyte cr_getstack(VM* vm, int32_t level, cr_debuginfo* di)
{
...
//       'defline', 'deflastline' in 'cr_debuginfo'.
//
// 'name' - function name
// 'type' - function type ('cript', 'main' or 'C')
...
static void getfuncinfo(VM* vm, Value cl, cr_debuginfo* di)
{
    const FnInfo* p = NULL;
    if(IS_NATIVE(cl)) {
        p = &AS_NATIVE(cl)->p;
        ...
        di->type = (p->defline == 0 ? "main" : "cript");
    }
...
// Sets 'line' in 'cr_debuginfo'.
//
// 'line' - current line number
static force_inline void auxgetline(Value cl, CallFrame* cf, cr_debuginfo* di)
{
    if(cf && cf->closure) {
...
static void getsrcinfo(Value cl, cr_debuginfo* di)
{
    FnInfo* p = (IS_NATIVE(cl) ? &AS_NATIVE(cl)->p : &AS_CLOSURE(cl)->fn->p);
