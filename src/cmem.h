/*
** cmem.h
** Functions for memory management
** See Copyright Notice in cscript.h
*/

#ifndef CMEM_H
#define CMEM_H


#include "climits.h"


#if defined(TRACEMEMORY)
#include <stdio.h>
#define premem(exp) \
        (printf("MEMCHANGE(%s:%d:%s) -> ", __FILE__, __LINE__, __func__), exp)
#else
#define premem(exp) exp
#endif


/* memory error */
#define csM_error(ts)   csPR_throw(ts, CS_ERRMEM);


#define csM_new(ts,t)           premem(csM_malloc_(ts, sizeof(t), 0))
#define csM_newarray(ts,s,t)    premem(csM_malloc_(ts, (s) * sizeof(t), 0))
#define csM_newobj(ts,tag,sz)   premem(csM_malloc_(ts, (sz), tag))

#define csM_free(ts,p)          premem(csM_free_(ts, p, sizeof(*(p))))
#define csM_freemem(ts,p,sz)    premem(csM_free_((ts), (p), (sz)))
#define csM_freearray(ts,p,n)   premem(csM_free_((ts), (p), (n)*sizeof(*(p))))

#define csM_reallocarray(ts,p,os,ns) \
        premem(((p) = csM_realloc_(ts, (p), (os)*sizeof(*(p)), \
                                              (ns)*sizeof(*(p)))))

#define csM_ensurearray(ts,p,s,n,e,l,w,t) \
        premem(((p) = csM_growarr_(ts, p, cast(int *, &(s)), n, sizeof(t), e, \
                                   l, w)))

#define csM_growarray(ts,p,s,n,l,w,t) \
        premem(csM_ensurearray((ts), p, s, n, 1, l, w, t))

#define csM_shrinkarray(ts,p,s,f,t) \
        premem(((p) = csM_shrinkarr_(ts, p, cast(int *, &(s)), f, sizeof(t))))


CSI_FUNC void *csM_malloc_(cs_State *ts, cs_umem size, int tag);
CSI_FUNC void *csM_realloc_(cs_State *ts, void *ptr, cs_umem osize,
                            cs_umem nsize);
CSI_FUNC void *csM_saferealloc(cs_State *ts, void *ptr, cs_umem osize,
                               cs_umem nsize);
CSI_FUNC cs_noret csM_toobig(cs_State *ts);
CSI_FUNC void csM_free_(cs_State *ts, void *ptr, cs_umem osize);
CSI_FUNC void *csM_growarr_(cs_State *ts, void *ptr, int *sizep, int len,
                           int elemsz, int ensure, int lim, const char *what);
CSI_FUNC void *csM_shrinkarr_(cs_State *ts, void *ptr, int *sizep, int final,
                             int elemsz);
CSI_FUNC int csM_growstack(cs_State *ts, int n);

#endif
