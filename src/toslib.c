...
static int os_clock(toku_State *T) {
    toku_push_number(C, ((toku_Number)clock()/(toku_Number)CLOCKS_PER_SEC));
    return 1;
}
...
static int get_bool_field(toku_State *T, const char *key) {
    int res = (toku_get_fieldstr(C, -1, key) == TOKU_T_NIL)
            ? -1
            : toku_to_bool(C, -1);
    toku_pop(C, 1);
    return res;
...
static int get_field(toku_State *T, const char *key, int dfl, int delta) {
    int isnum;
    int t = toku_get_fieldstr(C, -1, key); /* get field and its type */
...
static int os_time(toku_State *T) {
    time_t t;
    if (toku_is_noneornil(C, 0)) /* called without args? */
...
static int os_difftime (toku_State *T) {
    time_t t1 = t_checktime(C, 0);
    time_t t2 = t_checktime(C, 1);
    toku_push_number(C, (toku_Number)difftime(t1, t2));
    return 1;
}
