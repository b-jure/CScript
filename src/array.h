...
#define GSARRAY_PUSH(vm, objref)                                                                   \
    ({                                                                                             \
        if((vm)->gscap <= (vm)->gslen) {                                                           \
            ...
            (vm)->gscap      = MIN(GROW_ARRAY_CAPACITY(oldcap, ARRAY_INITIAL_SIZE), UINT24_MAX);   \
            (vm)->gray_stack = (Obj**)realloc((vm)->gray_stack, (vm)->gscap * sizeof(Obj*));       \
        }                                                                                          \
...
#define CARRAY_PUSH(chunk, constant, vm)                                                           \
    ({                                                                                             \
        if((chunk)->ccap <= (chunk)->clen) {                                                       \
            ...
            (chunk)->ccap      = MIN(GROW_ARRAY_CAPACITY(oldcap, ARRAY_INITIAL_SIZE), UINT24_MAX); \
...
#define CARRAY_FREE(chunk, vm)                                                                     \
    do {                                                                                           \
        gc_free(vm, (chunk)->constants, (chunk)->ccap, 0);                                         \
        CARRAY_INIT(chunk);                                                                        \
    } while(false)
...
#define GARRAY_PUSH(vm, global)                                                                    \
    ({                                                                                             \
        if((vm)->globcap <= (vm)->globlen) {                                                       \
            ...
            (vm)->globcap  = MIN(GROW_ARRAY_CAPACITY(oldcap, ARRAY_INITIAL_SIZE), UINT24_MAX);     \
...
#define GARRAY_FREE(vm)                                                                            \
    do {                                                                                           \
        gc_free(vm, (vm)->globvals, (vm)->globcap, 0);                                             \
        GARRAY_INIT(vm);                                                                           \
    } while(false)
...
#define ARRAY_NEW(name, type)                                                                      \
    ...
    {                                                                                              \
        self->data = (type*)gc_reallocate(self->vm, self->data, 0, cap * sizeof(type));            \
        self->cap  = cap;                                                                          \
    }                                                                                              \
                                                                                                   \
    ...
    {                                                                                              \
        ...
        gc_free(self->vm, self->data, self->cap, 0);                                               \
    }
